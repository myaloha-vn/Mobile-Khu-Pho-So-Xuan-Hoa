...
export function Header({
  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-40">
      ...
      {/* Breadcrumb */}
      <div className="px-4 sm:px-6 pb-3 flex items-center gap-1.5 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
        {breadcrumb.map((b, i) => (
          <span key={i} className="flex items-center gap-1.5 whitespace-nowrap">
            {i > 0 && <span className="text-[12.5px] leading-none text-slate-300">/</span>}
            {b.to ? (
              <button onClick={() => navigate(b.to!)}
                className="text-[12.5px] leading-none font-normal text-slate-400 hover:text-blue-500 transition-colors">
                {b.label}
              </button>
            ) : (
              <span className="text-[12.5px] leading-none font-normal text-slate-400">{b.label}</span>
            )}
          </span>
        ))}
      </div>
