...
export function WardMap({ hoods, highlight, onSelect }: {
  hoods: Hood[]; highlight: string | null; onSelect?: (code: string) => void;
}) {
      ...
      <div className="relative flex-1 min-w-0 rounded-xl overflow-hidden bg-slate-50 border border-slate-200">
        ...
        <div className="relative grid grid-cols-6 grid-rows-3 gap-2 p-3 h-full">
          {hoods.map((h) => {
            ...
            return (
              <button key={h.code} type="button" onClick={() => onSelect?.(h.code)}
